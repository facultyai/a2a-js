...
export class InMemoryTaskStore implements TaskStore {
  ...
  async list(params: ListTasksParams): Promise<ListTasksResult> {
    ...
    const lastUpdatedAfterDate = params.lastUpdatedAfter
      ? new Date(params.lastUpdatedAfter)
      : undefined;
    const pageTokenDate = params.pageToken
      ? new Date(Buffer.from(params.pageToken, 'base64').toString('utf-8'))
      : undefined;
